          ...
          <div key={rel.title} className="release-card">
            <div className="release-header">
              ...
              <div className="release-stats">
                <div className="release-revenue">${rel.totalRevenue?.toFixed ? rel.totalRevenue.toFixed(2) : Number(rel.totalRevenue).toFixed(2)}</div>
                <div className="release-sales">{rel.totalSales} sales</div>
                <button className="toggle-tracks-btn" onClick={() => toggle(rel.title)}>{expanded[rel.title] ? 'Hide tracks' : 'Show tracks'}</button>
              </div>
            </div>
            ...
            {expanded[rel.title] && (
              <div className="tracks-section">
                <div className="full-release-summary">
                  <div className="tracks-label">Full release/album sales</div>
                ...
                <div className="tracks-list">
                  {((tracksByRelease[rel.title] || []).slice().sort((a, b) => b.sales - a.sales)).map((t, idx) => (
                    <div key={`${t.title}-${idx}`} className="track-item">
                      ...
                      <div className="track-stats">{t.sales} · ${Number(t.revenue).toFixed(2)}</div>
                    </div>
                  ))}
